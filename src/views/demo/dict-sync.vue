              ...
              <div v-if="dictForm" class="dict-form">
                <el-form :model="dictForm" label-width="80px">
                  <el-form-item label="字典类型ID">
                    <el-input v-model="dictForm.dictTypeId" disabled />
                  </el-form-item>
            ...
            <div class="dict-component-demo">
              ...
              <div>
                <el-tag
                  v-for="item in dictStore.getDictItems('gender')"
                  :key="item.value"
                  :type="(item.color as 'primary' | 'success' | 'warning' | 'danger' | 'info') || undefined"
                  class="mr-2"
                >
                  {{ item.label }}
                </el-tag>
...
const loadMaleDict = async () => {
  const data = await DictAPI.getDictItemFormData(MALE_ITEM_ID);
  dictForm.value = data;
};
...
const saveDict = async () => {
  ...
  await DictAPI.updateDictItem(MALE_ITEM_ID, dictForm.value);
