...
/**
 * 生成主题色
 * @param colors 主题色映射（primary, success, warning, danger, info）
 * @param theme 主题类型
 */
export function generateThemeColors(colors: Record<string, string>, theme: ThemeMode) {
  const resolvedTheme = resolveThemeMode(theme);
  const result: Record<string, string> = {};

  // 为每个颜色生成深浅变体
  const colorNames = ["primary", "success", "warning", "danger", "info"];
